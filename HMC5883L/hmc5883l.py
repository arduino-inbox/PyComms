...
class HMC5883L:
    ...
    def setGain(self, value):
        self.i2c.write8(self.HMC5883L_RA_CONFIG_B, value << (
    ...
    def setMode(self, newMode):
        # use this method to guarantee that bits 7-2 are set to zero,
        # which is a
        # requirement specified in the datasheet
        self.i2c.write8(self.HMC5883L_RA_MODE, self.mode << (
        self.HMC5883L_MODEREG_BIT - self.HMC5883L_MODEREG_LENGTH + 1))
        self.mode = newMode # track to tell if we have to clear bit 7 after
        # a read

    def getHeading(self):
        packet = self.i2c.readBytesListS(self.HMC5883L_RA_DATAX_H, 6)
        if (self.mode == self.HMC5883L_MODE_SINGLE):
            self.i2c.write8(self.HMC5883L_RA_MODE,
                            self.HMC5883L_MODE_SINGLE << (
                            self.HMC5883L_MODEREG_BIT - self
